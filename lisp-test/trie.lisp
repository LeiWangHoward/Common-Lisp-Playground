...
;(load "F:/325lisp/homework/trie.lisp")


#|(defun add-word(word trie)
  (cond((equal(subseq word 1) "")
        (if(gethash(elt word 0)trie)
            (setf(gethash #\. (gethash(elt word 0) trie))(make-hash-table))
          (setf(gethash(elt word 0)(trie-b trie))(make-hash-table)
            (gethash #\.(gethash(elt word 0)trie))(make-hash-table))))
        (t(unless(gethash (elt word 0)trie)
            (setf(gethash(elt word 0)trie)
              (make-hash-table)))
           (add-word(subseq word 1)(gethash(elt word 0)trie)))))|#
