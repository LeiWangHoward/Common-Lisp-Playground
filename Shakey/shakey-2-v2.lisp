...
(defparameter *general-planning-kb*
  '(
    ...
    (<- (plan-for ?state1 ?goal (cons ?action ?actions))
        (action-for ?state1 ?state2 ?action) ;;state2 ->goal
...
(defparameter *shakey-2-kb*
  '(
    ...
    (<- (action-for (v2-state hall hall ?unlocked)
                    (v2-state ?gloc ?gloc ?unlocked) ;;room1 to ?gloc
                    (push-box hall ?gloc))
    ...
    ;; shakey at hall
    (<- (action-for (v2-state hall ?bloc ?unlocked) ;?sloc->hall
                    (v2-state ?sloc ?bloc ?unlocked)
                    (move-to ?sloc))
    ...
    (<- (action-for (v2-state ?sloc ?bloc ?unlocked)
                    (v2-state hall ?bloc-2 ?unlocked-2)
                    (move-to hall))
        (different ?sloc ?bloc)
        (not (connect ?sloc ?bloc)))

    ;; unlock room 
    (<- (action-for (v2-state hall ?room ?unlocked)
                    (v2-state hall ?room (cons ?room ?unlocked)) ;sloc-2 bloc-2
                    (unlock ?room))
        (different ?room hall)
        (not (member ?room ?unlocked)));hall needn't to be unlocked
))
