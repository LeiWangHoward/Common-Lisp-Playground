...
(defclass trie ()
  ...
  (:documentation "Represents a trie node."))
#| This module implements tries. A trie is a recursive data structure that maps lists of keys into values. Common key prefixes are shared. For example, given the following mappings: a => 1, aa => 2, ab => 3, ba => 4, we get the following trie (nodes are represented as [ value | branches...]): +--[ 2 | nil ] / +--[ 1 | a b ] / \ [ nil | a b ] +--[ 3 | nil ] \ +--[ nil | a ] \ +--[ 4 | nil ] Note that nil and b don't have mappings in the above trie. This implementation uses hash tables to store the mapping from branch keys into branch nodes. |# 
 (export 'trie) 
...
(defgeneric (setf get-trie) (val key-list trie)
  (:method (val (key-list list) (trie trie))
   (with-slots (value)
       (navigate-trie trie key-list :create-new-branches t)
     (setf value val))))

(defgeneric bulk-insert-trie (key-list trie &key map-fn)
  (:documentation "Inserts multiple items into the trie. The input list contains (key-list value) pairs. The optional map-fn can be supplied to map input list items.")
